...
> [!WARNING]
> Laravel Blocks has not been released. The package foundation, canonical document boundary, block registry, schema validator, Blade renderer, manifest bridge, compiled asset installer/distribution boundary, editor shell and command layer, current editing controls, top-level drag/drop, Document/List View, provider-backed Image/Video/File media UI, and default Paragraph/Heading/List/Quote/Code/Image/Video/File blocks exist. The complete editor UX, validation rule, and remaining built-in block catalog remain milestone targets.
...
The Blade component uses the same renderer path as the facade. Current rendering includes package-owned Paragraph, Heading, Bullet List, Ordered List, List Item, Quote, Code, Image, Video, and File views plus any application blocks registered with safe Blade views. The remaining built-in block catalog arrives in later batches.
