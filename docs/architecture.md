...
The implemented block-editor surface adds the first top-level block control path. The editor derives the current block from the Tiptap selection, avoids a hard active content stroke, shows one contextual toolbar with Transform To, a drag handle, Move Up/Down, common formatting, Link, and More, and exposes options for Duplicate, Insert before, Insert after, and Delete. The drag handle provides basic top-level pointer reorder with insertion feedback and JSON synchronization. These controls are limited to top-level blocks; nested wrappers, nested drag/drop, List View, keyboard reordering, multi-select, and complete block-specific controls remain later milestones.
