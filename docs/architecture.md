...
The implemented editor engine also exposes a shared internal selection and command layer around the mounted Tiptap instance. Command metadata reports active/enabled state and deterministic disabled reasons, while command execution routes focus, bold, italic, link, paragraph, heading, list, quote, code, top-level block duplicate/delete/insert/move, manifest block insertion, simple block-attribute updates, undo, and redo through one registry. This is the shared mutation boundary for the sticky header, contextual toolbar, menus, shortcuts, Inserter, slash commands, and Inspector controls.
...
The implemented contextual toolbar consolidates block and rich-text controls into one surface. It appears for the active top-level block, uses the shared command layer and UI primitives, exposes Transform To, Move Up/Down, Bold, Italic, Link, and More, updates canonical hidden JSON, hides while higher-priority overlays such as the header Inserter are open, and restores focus to the editor canvas after covered actions. Full mark coverage, keyboard shortcuts, mixed-state polish, nested contexts, and schema-filtered command availability remain later milestones.
...
The implemented block-editor surface adds the first top-level block control path. The editor derives the current block from the Tiptap selection, avoids a hard active content stroke, shows one contextual toolbar with Transform To, Move Up/Down, common formatting, Link, and More, and exposes options for Duplicate, Insert before, Insert after, and Delete. These controls are limited to top-level blocks; nested wrappers, drag/drop, List View, keyboard reordering, multi-select, and complete block-specific controls remain later milestones.

The implemented inserter surface reads the PHP-generated editor manifest payload and renders a sticky-header plus button that opens a searchable icon-grid Inserter with a Browse all affordance. Supported manifest entries currently map to bundled Tiptap nodes for `paragraph`, `heading`, `bulletList`, `blockquote`, and `codeBlock`; unsupported entries remain visible but disabled with a reason. Patterns, reusable blocks, recent/favorites, nested insertion rules, and async provider behavior remain later milestones.
