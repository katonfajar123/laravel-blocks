...
The implemented inserter surface reads the PHP-generated editor manifest payload and renders one standalone plus button after the final top-level block. It opens a searchable icon-grid Inserter with a Browse all affordance and inserts after that final block. Package defaults now register production PHP definitions and Blade renderers for every currently insertable Text entry: `paragraph`, `heading`, `bulletList`, `orderedList`, `blockquote`, and `codeBlock`; unsupported entries remain visible but disabled with a reason, while manifest entries with `supports.inserter: false` are hidden for structural nodes. Patterns, reusable blocks, recent/favorites, nested insertion rules, and async provider behavior remain later milestones.
