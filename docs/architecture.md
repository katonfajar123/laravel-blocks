...
The implemented contextual toolbar consolidates block and rich-text controls into one intent-gated surface. It stays hidden during ordinary collapsed-cursor typing, appears in inline mode for non-empty text selections, appears in empty mode as a compact transform affordance for empty focused text blocks, and appears in hover mode as a lightweight drag handle plus options button for top-level blocks. It uses the shared command layer and UI primitives, exposes Transform To, Move Up/Down, Bold, Italic, Link, and More through the relevant modes, updates canonical hidden JSON, hides while higher-priority overlays such as the header Inserter are open, and restores focus to the editor canvas after covered actions. Full mark coverage, keyboard shortcuts, mixed-state polish, nested contexts, explicit block selection, and schema-filtered command availability remain later milestones.
...
The implemented block-editor surface adds the first top-level block control path. The editor derives the current block from the Tiptap selection for command state, derives a separate top-level block from pointer hover for hover controls, avoids a hard active content stroke, and does not show the block frame during normal typing. Hover exposes a drag handle and options menu for Transform To, Move Up/Down, Duplicate, Insert before, Insert after, and Delete; selected text exposes only inline formatting/link controls. The drag handle provides basic top-level pointer reorder with insertion feedback and JSON synchronization. These controls are limited to top-level blocks; nested wrappers, nested drag/drop, keyboard reordering, multi-select, explicit block selection, and complete block-specific controls remain later milestones.
...
The implemented inserter surface reads the PHP-generated editor manifest payload and renders a sticky-header plus button that opens a searchable icon-grid Inserter with a Browse all affordance. Supported manifest entries currently map to bundled Tiptap nodes for `paragraph`, `heading`, `bulletList`, `orderedList`, `blockquote`, and `codeBlock`; unsupported entries remain visible but disabled with a reason, while manifest entries with `supports.inserter: false` are hidden for structural nodes. Patterns, reusable blocks, recent/favorites, nested insertion rules, and async provider behavior remain later milestones.
