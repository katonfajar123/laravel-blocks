...
The current implementation includes the first manifest-generated Inspector sidebar for the selected top-level block, but it is closed by default and opens only from the explicit settings control in the sticky header. It renders the Content, Design, and Advanced tabs, derives basic controls from declarative manifest fields, updates simple `attrs.*` values through the shared command registry, and keeps canonical hidden JSON synchronized. Image exposes URL, alternative-text, and title Content controls plus a provider-backed Media Library entry point shared with the contextual toolbar. The modal browses/searches images, uploads by input or drop with progress/cancel/retry states, restores focus, and applies URL/alt in one undoable transaction. Clearing a nullable control restores `null`, and an Image without `src` remains a visible editor placeholder without emitting a broken frontend element. The complete Field Engine, caption/link controls, schema-driven design settings, inline validation feedback, custom Inspector extension points, document settings, and polished responsive drawer behavior remain later milestones.
