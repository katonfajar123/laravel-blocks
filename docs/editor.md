...
The current implementation includes a manifest-driven trailing Inserter and slash command menu. These surfaces read the editor manifest payload, group or filter blocks by category/search query, insert supported Tiptap nodes through the shared command layer, and show disabled unsupported manifest blocks with a reason. One standalone rounded plus button sits after the final top-level block and opens a compact rounded icon-grid Inserter with search, soft elevation, viewport-aware above/below placement, and a Browse all affordance; it is not part of the header or contextual toolbar. The slash menu opens as a compact keyboard-first rounded popover from `/` in an empty top-level text block. Slash commands support query typing, ArrowUp/ArrowDown, Enter, Escape, Backspace, and pointer insertion, and replace the triggering empty block with the selected supported node. The package-default manifest and bundle currently map `paragraph`, `heading`, `bulletList`, `orderedList`, `blockquote`, `codeBlock`, `image`, and `video`; package manifest entries marked `supports.inserter: false` are hidden from the Inserter/slash surfaces for structural nodes such as `listItem`. Patterns, reusable blocks, recent/favorites, nested insertion rules, Home/End behavior, IME polish, and async provider behavior remain later milestones.
...
The current implementation includes basic top-level block controls gated by explicit user intent. A collapsed cursor, including one inside an empty block, does not show the contextual toolbar. Hovering a top-level block shows one lightweight rounded handle; activating it opens the single complete rounded floating contextual toolbar with Transform To, drag/move controls, applicable block-specific controls, thin group dividers, amber active states, and an options menu for Move Up/Down, Duplicate, Insert before, Insert after, and Delete. Heading blocks expose H1-H6 in that toolbar, supported text blocks include Bold, Italic, Highlight, and Link, and Code omits unsupported marks. These actions execute through the shared command registry, keep handle/menu handoff stable, avoid forced scroll jumps, and update canonical hidden JSON.
...
The current implementation routes Bold, Italic, Highlight, and Link through the unified contextual toolbar rather than a second text-only toolbar. A non-empty text selection opens the same complete toolbar composition used for block controls, with only the groups valid for the selected block. Highlight uses a stable yellow `<mark>` output on the basic path; custom highlight colors remain a later mark-control task. Link editing uses selection state, routes actions through the shared command layer, updates canonical hidden JSON, and restores focus to the editor canvas.

The Link control opens a selection-anchored popover with URL input, open-in-new-tab toggle, Apply, Unlink, inline validation feedback, Escape cancellation, and preserved-selection mutation. The current provider boundary validates safe external, root-relative, and anchor links in the editor; server-side validation still needs relative/anchor parity. Internal-link search/autocomplete, remaining marks, keyboard shortcuts, mixed-selection state, and schema-filtered mark availability remain later milestones.
...
The current implementation includes the first manifest-generated Inspector sidebar for the selected top-level block, but it is closed by default and opens only from the explicit settings control in the sticky header. It renders the Content, Design, and Advanced tabs, derives basic controls from declarative manifest fields, updates simple `attrs.*` values through the shared command registry, and keeps canonical hidden JSON synchronized. Image exposes URL, alternative-text, and title controls; Video exposes source URL, poster URL, and accessible-title controls. Both use the same provider-backed Media Library entry point from the Inspector or contextual toolbar. The modal filters browse/upload results by the active block's MIME family, restores focus, and applies the source plus supported provider metadata in one undoable transaction. Clearing a nullable control restores `null`; an Image or Video without `src` remains a visible editor placeholder without emitting a broken frontend element. The complete Field Engine, Image caption/link controls, Video caption tracks and poster picker, schema-driven design settings, inline validation feedback, custom Inspector extension points, document settings, and polished responsive drawer behavior remain later milestones.
...
The current implementation includes the internal selection snapshot and shared command registry plus a sticky header, one intent-gated adaptive rounded contextual toolbar, Transform To/options menus, link popover, a compact viewport-aware trailing manifest Inserter, basic slash commands, basic generated Inspector controls, a basic top-level Document/List View, and visible header Undo/Redo controls. Floating toolbar/popover surfaces use large radii, soft shadow, subtle border, muted inactive controls, amber active state, and stroke-based SVG icons. Undo and Redo execute through the command registry from the sticky header and from Ctrl/Cmd+Z and Ctrl/Cmd+Shift+Z; Ctrl/Cmd+Shift+2, Ctrl/Cmd+Shift+3, and Ctrl/Cmd+Shift+4 map to Heading 2, 3, and 4. The default editor surface is full-width and clean; collapsed typing does not open block chrome, the standalone plus remains after the final block, and the Inspector and List View do not open automatically. The full shortcut map, nested controls, advanced drag/drop, and complete block operation UI remain later milestones.
